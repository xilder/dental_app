...
def validate_email(email):
    ...
    print(user)
    if user:
        return make_response(jsonify(valid=False, message=f'{email} has already been registered'), 200)
    else:
...
def validate_username(username):
    ...
    print(user)
    if user:
        return make_response(jsonify(valid=False, message=f'{username} has already been registered'), 200)
    else:
        return make_response(jsonify(valid=True, message='Valid username'), 200)
